...
print("Importing custom pix2pix")
...
def unet_generator(output_channels, norm_type='batchnorm'):
  ...
  down_stack = [
      ...
      downsample(32, 4, norm_type, apply_norm=False),  # (bs, 64, 64, 128)
      downsample(64, 4, norm_type),  # (bs, 32, 32, 256)
      downsample(128, 4, norm_type),  # (bs, 16, 16, 512)
      downsample(128, 4, norm_type),  # (bs, 8, 8, 512)
      downsample(128, 4, norm_type),  # (bs, 4, 4, 512)
      downsample(128, 4, norm_type),  # (bs, 2, 2, 512)
      downsample(128, 4, norm_type),  # (bs, 1, 1, 512)
  ]

  up_stack = [
      upsample(128, 4, norm_type, apply_dropout=False),  # (bs, 2, 2, 1024)
      upsample(128, 4, norm_type, apply_dropout=False),  # (bs, 4, 4, 1024)
      upsample(128, 4, norm_type, apply_dropout=False),  # (bs, 8, 8, 1024)
  ...
  inputs = tf.keras.layers.Input(shape=[None, None, N_CHANNELS])
  x = inputs
...
def discriminator(norm_type='batchnorm', target=True):
  ...
  if target:
    tar = tf.keras.layers.Input(shape=[None, None, N_CHANNELS], name='target_image')
    x = tf.keras.layers.concatenate([inp, tar])  # (bs, 256, 256, channels*2)

  down1 = downsample(16, 4, norm_type, False)(x)  # (bs, 128, 128, 64)
  down2 = downsample(32, 4, norm_type)(down1)  # (bs, 64, 64, 128)
  down3 = downsample(64, 4, norm_type)(down2)  # (bs, 32, 32, 256)

  zero_pad1 = tf.keras.layers.ZeroPadding2D()(down3)  # (bs, 34, 34, 256)
  conv = tf.keras.layers.Conv2D(
      128, 4, strides=1, kernel_initializer=initializer,
...
class Pix2pix(object):
  ...
  def __init__(self, epochs, enable_function):
    ...
    self.generator = unet_generator(output_channels=N_CHANNELS)
